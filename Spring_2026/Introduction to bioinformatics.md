...
We have already dwelled a little bit on this. The terminal is just a tool to connect commands with the machine, just like a keyboard and mouse. Simple! You have already experienced how most of the linux machine look like: there is no graphical user interface (GUI) between you and the server. GUI's do exist for personal Linux based machines. We use ‘commands’ to communicate with the server/virtual machine. A 'command' can be synonymous to 'transfer a file from one folder to another folder' in your own computer when you have access to a mouse or touchpad.
...
Most likely you will not see anything. Just an empty folder. Lets make a folder inside this.
...
Example: cd .. (Take you one level up toward the directory called **root**) or cd /
...
```
Yes, it is analogous to the **root** of a tree. All the other directories in the server are branching from this directory. 
...
Confirm the result by 'ls dir1'. Or just run ls in the current directory. If it is not present in the current directory, then it is moved to dir1. The mv command physically moves a file from one location to another.

You can use mv command to move more than one element from a directory. Last parameter in the mv command will be the one which receives the other elements. 
...
When you have few lines, you can use less instead. Example: ls | less
...
Our goal is to make it here for today, but if we have more time, you can try out the following commands and their usage: 'zcat', 'unzip', 'gzip' and 'history'. Find out what they do. These commands will give different information about the system, unzip a zipped folder and so on.
